...
describe('통합: 리듀서로 5홀 샘플 라운드 진행 (수기 계산 대조)', () => {
  it('전원동타 배판, 니어+버디, 스킵 리셋, 양파 시나리오', () => {
    ...
    const state = run([
      ...
      // 1번 홀: 전원 파 동타 → 돈 이동 없음
      ...enterHole(1, 4, [5, 5, 5, 5]),
      // 2번 홀: 배판을 직접 선택 (×2), [5,6,7,8]
      ...enterHole(2, 5, [5, 6, 7, 8], [{ type: 'SET_MULTIPLIER', holeNo: 2, multiplier: 2 }]),
      // 3번 홀 파3: 니어 조건을 이 홀에서 정함 (정액 5천) + 배판 ×2 직접 선택, p1 버디+니어
